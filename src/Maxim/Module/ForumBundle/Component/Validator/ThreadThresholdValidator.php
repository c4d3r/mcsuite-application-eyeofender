...
class ThreadThresholdValidator extends ConstraintValidator
{
    ...
    public function validate($object, Constraint $constraint)
    {
        ...
        $lastThread = $this->em->getRepository("MaximModuleForumBundle:Thread")->findLatestThread($user);

        if(isset($lastThread[0]) && (false === $this->securityContext->isGranted('ROLE_STAFF')))
        {
            ...
            {
                $this->context->addViolation($constraint->threadThresholdMessage, array(
                    '%time%' => $this->threshold,
